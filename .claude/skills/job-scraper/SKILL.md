...
name: job-scraper
description: >
  Scrapes job sites (LinkedIn + Google site-searches) for new positions matching your profile. Deduplicates across runs.
...
This skill searches job sites (primarily LinkedIn and company career pages via Google site-search) using targeted queries based on your profile, deduplicates against previously seen jobs and the application tracker, and presents new matches with a quick fit assessment.
...
For each search:
- Use `WebSearch` with site-specific queries (linkedin.com/jobs, Indeed, company career pages via `site:` search, etc.)
